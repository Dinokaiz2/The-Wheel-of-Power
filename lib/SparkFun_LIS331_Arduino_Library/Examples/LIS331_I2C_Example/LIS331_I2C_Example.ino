...
void setup() 
{
  ...
  Wire.begin();
  accel.setI2CAddr(0x19);    // This MUST be called BEFORE .begin() so 
                          //  .begin() can communicate with the chip
  accel.begin(LIS331::USE_I2C); // Selects the bus to be used and sets
...
void loop() 
{
  ...
  {
    loopTimer = millis();
    accel.readAxes(x, y, z);  // The readAxes() function transfers the
                           //  current axis readings into the three
                           //  parameter variables passed to it.
    Serial.println(x);
    Serial.println(y);
    Serial.println(z);
    Serial.println(accel.convertToG(6,x)); // The convertToG() function
    Serial.println(accel.convertToG(6,y)); // accepts as parameters the
    Serial.println(accel.convertToG(6,z)); // raw value and the current
